...
class model_category extends model_base_class {
    ...
    public function list_categories() 
    {
        ...
        $sql = "SELECT site_category_id, site_category_title FROM site_categories ORDER BY site_category_title";
        $results = $this->exec_sql($sql);
        ...
        {
            $category = array();
            $category['site_category_id'] = $result['site_category_id'];
            $category['site_category_title'] = $result['site_category_title'];
    ...
    public function get_videos_by_playlist($playlist_id)
    {
        $sql = "SELECT site_video_id, site_video_title FROM site_videos WHERE site_playlist_id = " . $this->escape_string($playlist_id) . " ORDER BY site_video_index, site_video_title";
        $results = $this->exec_sql($sql);
        ...
        {
            $video = array();
            $video['site_video_id'] = $result['site_video_id'];
            $video['site_video_title'] = $result['site_video_title'];
            $videos[] = $video;
        }
    ...
    public function add_new_video($title, $type, $description, $location, $minutes, $seconds, $category_id, $playlist_id, $insert_at)
    {
        //TODO: HANDLE THE INSERT AT
        $sql = "INSERT INTO site_videos (site_video_title, site_video_type_id, site_video_description, site_video_file_location, site_video_minutes, site_video_seconds, site_category_id, site_playlist_id, site_video_date_added) " .
               " VALUES (" . 
                $this->escape_string($title) . "," . 
                $this->escape_string($type) . "," . 
                $this->escape_string($description) . "," . 
                $this->escape_string($location) . "," . 
                $this->escape_string($minutes) . "," . 
                $this->escape_string($seconds) . "," . 
                $this->escape_string($category_id) . "," . 
                $this->escape_string($playlist_id) . ", 
                CURDATE())";
        $this->exec_sql($sql);        
    }
}
